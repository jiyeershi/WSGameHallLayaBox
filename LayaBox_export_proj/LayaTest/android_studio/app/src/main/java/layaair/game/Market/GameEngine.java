...
import demo.Hall2GameInterface;
...
public class GameEngine implements IPlugin {
    ...
    public void game_plugin_init() {
         ...
         * 根据gameId设置参数
         */
//        String gameId = Hall2GameInterface.getBo().doQueryGameId();
//        mLayaGameEngine.setSoPath(WSGameSDKApplication.getGameApplicationInstance().getGameSoDir(gameId)+"/");
//        mLayaGameEngine.setSoFile("liblayaair.so");

        mLayaGameEngine.setInterceptKey(true);
        mLayaGameEngine.onCreate();
        LayaConch5 tmp = (LayaConch5) mLayaGameEngine;
        Log.e(TAG, "参数数据：SoPath=" + tmp.getSoPath() + " jarfile=" + tmp.getJarFile() + " appcache=" + tmp.getAppCacheDir());
    }
